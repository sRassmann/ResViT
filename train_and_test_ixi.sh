PRETRAIN="res_cnn_10k_ixi"
FINETUNE="resvit_20k_ixi"
INF_BATCH_SIZE=4
...
CUDA_VISIBLE_DEVICES=$DEVICE python train.py --gpu_ids 0 --model resvit_many --which_model_netG resvit --lambda_A 100 \
  --dataset_mode aligned --norm batch --pool_size 0 --output_nc 1 --input_nc 3 \
  --checkpoints_dir /groups/ag-reuter-2/users/rassmanns/diffusion/ResViT/checkpoints --name $FINETUNE --config ixi.yml \
  --niter 5000 --niter_decay 5000 --save_epoch_freq 100 \
  --pre_trained_transformer 1 --pre_trained_resnet 1  --batchSize 24 \
  --pre_trained_path /groups/ag-reuter-2/users/rassmanns/diffusion/ResViT/checkpoints/$PRETRAIN/latest_net_G.pth --lr 0.0002  # diverged with stated higher LR
...
CUDA_VISIBLE_DEVICES=$DEVICE python test.py --name $FINETUNE --gpu_ids 0 --model resvit_many --which_model_netG resvit \
  ...
  --serial_batches --checkpoints_dir /groups/ag-reuter-2/users/rassmanns/diffusion/ResViT/checkpoints --which_epoch latest --config ixi.yml \
